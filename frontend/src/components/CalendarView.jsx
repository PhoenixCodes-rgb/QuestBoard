    ...
    <div className="mt-8 space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={goToToday}
            className="ml-2 rounded-full bg-white px-4 py-2 text-xs font-medium text-gray-600 shadow-sm transition hover:bg-gray-100"
          >
            Today
          </button>
        </div>
      ...
      <div className="overflow-hidden rounded-3xl bg-white p-6 shadow-sm">
        {/* Days of week header */}
        <div className="grid grid-cols-7 gap-1 border-b border-gray-100 pb-3 text-center text-xs font-semibold text-gray-400">
          <span>Sun</span>
          <span>Mon</span>
          <span>Tue</span>
          <span>Wed</span>
            ...
            return (
              <button
                ...
                className={`relative flex min-h-16 flex-col items-center justify-between rounded-2xl p-2 transition sm:min-h-20 ${
                  isSelected
                    ? "bg-[#193b27] text-white shadow-md"
                    : isTodayCell
                    ? "bg-green-50 text-green-800 ring-2 ring-green-600"
                    : item.isCurrentMonth
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <motion.button
            ...
            className="flex items-center gap-2 self-start rounded-full bg-[#193b27] px-4 py-2.5 text-xs font-medium text-white shadow-sm transition hover:bg-green-800 sm:self-auto"
          >
            <FiPlus />
        ...
        {selectedDayTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl bg-white px-6 py-12 text-center shadow-sm">
            ...
            <button
              onClick={() => onCreateTaskForDate(selectedDayKey)}
              className="mt-4 rounded-full bg-[#193b27] px-4 py-2 text-xs font-medium text-white transition hover:bg-green-800"
            >
              + Add a task
            </button>
          </div>
